...
// 账户仅允许字母、数字、下划线（手机号/用户名）；密码不允许汉字与空格
const CHINESE_REG = /[\u4e00-\u9fa5]/;
const ACCOUNT_ALLOWED_REG = /^[A-Za-z0-9_]*$/;
...
export default {
  ...
  data() {
    const validateAccount = (rule, value, callback) => {
      ...
      if (account.length > ACCOUNT_MAX_LENGTH) {
        callback(new Error(`账户长度不能超过${ACCOUNT_MAX_LENGTH}位`));
        return;
      }
      if (CHINESE_REG.test(account) || !ACCOUNT_ALLOWED_REG.test(account)) {
        callback(new Error('账户不支持特殊字符'));
        return;
      }
      callback();
    };
    const validatePassword = (rule, value, callback) => {
      ...
      if (password.length > PASSWORD_MAX_LENGTH) {
        callback(new Error(`密码长度不能超过${PASSWORD_MAX_LENGTH}位`));
        return;
      }
      if (CHINESE_REG.test(password)) {
        callback(new Error('密码不支持汉字'));
        return;
      }
      if (/\s/.test(password)) {
        callback(new Error('密码不支持空格'));
        return;
      }
      callback();
    };
  ...
  methods: {
    ...
    onPasswordInput(value) {
      ...
      this.loginForm.password = filtered;
    },
    async handleLogin() {
        ...
        if (valid) {
          ...
          try {
            ...
            if (userInfo && userInfo.menuList && userInfo.menuList.length > 0) {
              ...
              const notFoundRoute = this.$router
                .getRoutes()
            ...
            setTimeout(() => {
              // 检查路由是否存在
              const routeExists = this.$router
                .getRoutes()
                .some((route) => route.path === redirectPath || route.name === redirectPath);
              if (routeExists) {
                ...
                if (availableRoutes.length > 0) {
                  ...
                  this.$router.replace(fallbackPath).catch(() => {
                    window.location.href =
                      window.location.origin + '/#/' + fallbackPath.replace(/^\//, '');
                  });
                } else {
