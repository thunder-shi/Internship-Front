<template>
  <DlgBasic ref="dlgBasicRef" :default-props="defaultProps" :dlgbasic-spec-confirm="submit">
    <template #mainForm>
      <el-upload ref="uploadRef" class="upload-demo" drag action :limit="1" :show-file-list="true" :http-request="()=>{}" :on-exceed="handleExceed" :before-upload="beforeUpload" :on-remove="()=>file.value={}">
        ...
        <template #tip>
          ...
          <div v-if="isNeedExport" class="el-upload__tip">请先进行<span class="download">"导出"</span>或<span class="download">"全部导出"</span>操作获取模板</div>
...
const emit = defineEmits(['selfImport', 'ex-material'])
