  ...
  -->
<template>
  <DlgBasic ref="dlgBasicRef" v-model:default-props="defaultProps.defaultDBProps" :dlgbasic-confirm="confirm" :dlgbasic-submit="submit" :dlgbasic-spec-confirm="specConfirm" @close-dialog="onCloseDialog" @open-dialog="openDialog">
    <template #mainForm>
      <el-form ref="formPanelRef" :rules="formRules" :model="form" label-suffix=":" :label-width="labelWidth">
        ...
        <div v-for="(item, index) in formItems" :key="index">
          <el-form-item v-if="item.type!=='textarea'" :prop="item.field" :label="item.name">
            ...
            <!-- 关联数据选择框 -->
            <SimpleSelect v-else-if="item.type==='select'" :ref="'smpSel'+item.field" v-model="form[item.field]" :field="item.field" :select-label="item.selectLabel" :multiple="item.multiple" v-model:form="form" :key-words="item.keyWords" :search-key="form[item.searchKeys]" :reg-key="item.regKey" :disabled="item.disabled" :placeholder="item.placeholder?item.placeholder:'请选择'" :auto-select="item.autoSelect" @update-value="onSimpleSelectChange" @init-finish="simpleSelectInitFinish" />
            <!-- 级联选择框 -->
...
const emit = defineEmits(['update-record', 'close-dialog', 'open-dialog', 'simple-select-change', 'simple-select-init-finish', 'tree-select-change'])
...
const formItems = computed(() => {
  return props.defaultProps.formItems ? props.defaultProps.formItems : [
...
const formRules = computed(() => {
  return props.defaultProps.formRules ? props.defaultProps.formRules : { name: [{ required: true, message: '名称不能为空', trigger: 'blur' }] }
})
...
function verifyValid() {
  ...
  nextTick(() => {
    if (formPanelRef.value && dlgBasicRef.value) {
      formPanelRef.value.validate((valid) => {
...
function onTreeSelectChange(val, field) {
  form[field] = val
  emit('tree-select-change', val, field, form)
}
