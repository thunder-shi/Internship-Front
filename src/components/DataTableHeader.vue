    ...
    <div v-if="showTopButtons" class="crud-opts">
      <el-row type="flex" justify="space-between">
        ...
        <el-button
          ...
          :icon="CirclePlus"
          :disabled="button.more1.disabled"
        ...
        <el-button
          ...
          :disabled="
            ...
            (selectedColumns.length === 1 && !isButtonVisible('update', selectedColumns[0]))
          "
        ...
        <el-button
          ...
          :disabled="
            ...
            (selectedColumns.length > 0 &&
              selectedColumns.some((row) => !isButtonVisible('delete', row)))
          "
        ...
        >
        <el-button
          v-if="button.more3.show"
          :type="button.more3.type"
          :icon="Setting"
          @click="more3Click(selectedColumns)"
          >{{ button.more3.name }}</el-button
        >
        <el-button
          v-if="button.more4.show"
          :type="button.more4.type"
          :icon="Pointer"
          @click="more4Click(selectedColumns)"
          >{{ button.more4.name }}</el-button
        >
        <!-- 全部提交按钮（仅 more5 支持 submitAll） -->
        <el-button
          v-if="button.more5.show"
          :type="button.more5.type"
          :icon="Finished"
          :disabled="button.more5.disabled"
          @click="more5Click(selectedColumns)"
          >{{ button.more5.name }}</el-button
        >
        <el-button
        ...
        >
        <div
          v-if="button.audit.show && button.audit.showHeaderBtn !== false"
        ...
        >
          ...
          <el-dropdown
            ...
            <template #dropdown>
              <el-dropdown-menu>
                <el-dropdown-item v-if="button.audit.showPass" :command="CONSTANT.AUDIT_STATUS.PASS"
                  >批量{{ CONSTANT.AUDIT_STATUS.PASSNAME }}</el-dropdown-item
                >
                <el-dropdown-item
                  v-if="button.audit.showNotPass"
                  :command="CONSTANT.AUDIT_STATUS.NOTPASS"
                  >批量{{ CONSTANT.AUDIT_STATUS.NOTPASSNAME }}</el-dropdown-item
                >
                <el-dropdown-item v-if="button.audit.showBack" :command="CONSTANT.AUDIT_STATUS.BACK"
                  >批量{{ CONSTANT.AUDIT_STATUS.BACKNAME }}</el-dropdown-item
                >
              </el-dropdown-menu>
            </template>
          </el-dropdown>
...
const button = computed(() => {
  const btn = {
    ...
    more4: { show: false, name: '更多操作4', type: 'warning' },
    more5: { show: false, name: '更多操作5', type: 'warning' },
    audit: {
      show: false,
      name: '审核',
      type: 'danger',
...
// 更多内容4
async function more4Click(row) {
  emit('more4-click', row);
}

async function more5Click(row) {
